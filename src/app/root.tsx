...
import { postToParent, isTrustedEventOrigin } from '@/lib/securePostMessage';
...
function InternalErrorBoundary({ error: errorArg }: Route.ErrorBoundaryProps) {
  ...
  const { buttonProps: showLogsButtonProps } = useButton(
    {
      onPress: useCallback(() => {
        postToParent({ type: 'sandbox:web:show-logs' });
      }, []),
    },
  ...
  const { buttonProps: fixButtonProps } = useButton(
    {
      onPress: useCallback(() => {
        postToParent({ type: 'sandbox:web:fix', error: serializeError(error) });
        setIsOpen(false);
      }, [error]),
      isDisabled: !error,
    },
...
const useHandshakeParent = () => {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      if (!isTrustedEventOrigin(event)) return;
      if (event.data.type === 'sandbox:web:healthcheck') {
        postToParent(healthyResponse);
      }
    };
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data.type === 'sandbox:web:refresh:request') {
        ...
        postToParent({ type: 'sandbox:web:refresh:complete' });
      }
    };
    window.addEventListener('message', handleMessage);
    return () => {
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('message', handleMessage);
    postToParent({ type: 'sandbox:web:ready' });
    return () => {
      window.removeEventListener('message', handleMessage);
    };
  }, [navigate]);
  ...
  useEffect(() => {
    if (pathname) {
      postToParent({ type: 'sandbox:web:navigation', pathname });
    }
  }, [pathname]);
  return (
    <html lang="tr">
